...
local function make_just_one_big_circuit(links_seq)
    local last_link    = links_seq[1]
    local circuit_set  = {}
    for _, link in ipairs(links_seq) do
        if not (circuit_set[link[1]] and circuit_set[link[2]]) then
            last_link = link
            circuit_set[link[1]] = true
            circuit_set[link[2]] = true
        end
...
-- step 2
local link = make_just_one_big_circuit(links_seq)
